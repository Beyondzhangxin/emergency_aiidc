...
<body>
    ...
    <div class="mainbox">
        ...
        <p class="content_tab_box">
            <a class="content_tab " href="list.do">政府端</a>
            <a class="content_tab content_tab1" href="companyList.do">企业端</a>
        </p>
        <!--数据表格-->
        <div class="contentbox">
            <p class="content_head_form">
                ...
                <span class="btnbox">
                    ...
                    <a class="btn2 btn_check" href="javascript:checkEmMaterialById()">查看</a>
            ...
            <div class="searchbox">
           		<div class="search_item">
                    ...
                    <input type="text" class="companyName txtinput" name="companyName">
                </div>
                <div class="search_item">
			...
			success:function(data){	
				if(data.status=="0000000"){
					...
					$.each(data.rows,function(i,n){	
						...
						html+='<tr data-sub='+sub+'>'
						+'<td><input type="checkbox" value="'+n.materialId+'" name="EmMaterialConpList"></td>'
						+'<td>'+((dataList.page-1)*dataList.rows+i+1)+'</td>'
						+'<td><a class="link_wz" href="conpDetails.do?companyId='+n.companyId+'" target="_blank">'+n.companyName+'</a></td>'
						+'<td><a class="link_wz" href="conpEmDetails.do?materialId='+n.materialId+'" target="_blank">'+n.materialName+'</a></td>'
						+'<td>'+n.classifyName+'</td>'
						+'<td>'+n.typeName+'</td>'
						+'<td>'+n.num+'</td>'
						+'<td class="expiryDate">'+n.expiryDateStr+'</td>'
						+'<td class="changeTime">'+n.createTimeStr+'</td></tr>'
					})		
					$("tbody").html(html);
...
<script type="text/javascript">
	function checkEmMaterialById() {
		var materialIds =[];
		$("input[name='EmMaterialConpList']:checked").each(function(){
			materialIds.push($(this).val());
		});
		if(materialIds.length >0){
			var materialId=materialIds[(materialIds.length-1)];
			var url="conpEmDetails.do?materialId="+materialId;
			window.open(url);
		}
	}
</script>
